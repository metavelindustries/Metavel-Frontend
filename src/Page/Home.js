    ...
    >
      {valve.images && valve.images.length > 0 && (
        <img 
          ...
          alt={valve.name} 
          className="h-[13rem] w-full object-cover rounded-md mb-3"
        />
      )}
      <h3 className="text-lg font-semibold text-gray-800 mb-1">{valve.name}</h3>
      <p className="text-sm text-gray-600 mb-2 line-clamp-2">{valve.metaDescription}</p>
      <Link to={`/valve/${valve._id}`} onClick={handleCategoryClick}> 
        <button className="mt-2 bg-blue-900 text-white px-3 py-1.5 text-sm rounded hover:bg-blue-700 transition-colors duration-300">
          View Details
        </button>
      </Link>
    </div>
